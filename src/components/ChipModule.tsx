...
const ChipModule: React.FC<ChipModuleProps> = ({
  ...
  const getModuleIcon = () => {
    switch (type) {
      ...
      case 'bus': return <Wifi className="w-6 h-6" />;
    ...
    switch (type) {
      ...
      case 'otp':
        return [
          ...
          { name: 'Access Control', status: currentStage >= 2 ? 'active' : 'idle', position: { x: 0, y: 1 } }
        ];
      case 'crypto':
        return [
      ...
      case 'flash':
        return [
          ...
          { name: 'Address Buffer', status: currentStage >= 3 ? 'active' : 'idle', position: { x: 0, y: 1 } },
          { name: 'Data Buffer', status: currentStage >= 3 ? 'active' : 'idle', position: { x: 1, y: 1 } }
        ];
      case 'cpu':
        return [
