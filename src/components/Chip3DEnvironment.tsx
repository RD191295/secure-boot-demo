  ...
  return (
    <div className="relative w-full h-screen bg-gray-900 overflow-hidden">
      ...
      <div className="absolute top-4 right-4 z-20 flex flex-col gap-2">
        <button onClick={() => setZoom(prev => Math.min(3, prev * 1.2))} className="p-2 bg-gray-800 text-white rounded hover:bg-gray-700" title="Zoom In">
          <ZoomIn size={16} />
        </button>
        <button onClick={() => setZoom(prev => Math.max(0.3, prev * 0.8))} className="p-2 bg-gray-800 text-white rounded hover:bg-gray-700" title="Zoom Out">
          <ZoomOut size={16} />
        </button>
        <button onClick={resetView} className="p-2 bg-gray-800 text-white rounded hover:bg-gray-700" title="Reset View">
          <RotateCcw size={16} />
        </button>
      </div>
        ...
        >
          ...
          >
            ...
            {buses.map((bus, index) => {
              ...
              const { from, to } = getConnectionPoints(fromModule, toModule);
